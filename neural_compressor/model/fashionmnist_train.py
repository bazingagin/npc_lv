...
import utils.torch.rand_comb as random
    ...
    def __init__(self, xs=(3, 32, 32), nz=1, zchannels=16, nprocessing=1, kernel_size=3, resdepth=2,
        ...
        if root_process:
            ...
            log_dir = os.path.join(
                'runs/fashionmnist/', current_time + '_' + socket.gethostname() + tag)
...
def test(model, device, epoch, ema, data_loader, tag, root_process):
    ...
    if root_process:
        ...
        if elbo < model.best_elbo and not np.isnan(elbo):
            ...
            if not os.path.exists(f'params/fashionmnist/'):
                os.makedirs(f'params/fashionmnist/')
            torch.save(model.state_dict(), f'params/fashionmnist/{tag}')
            if epoch % 25 == 0:
                torch.save(model.state_dict(), f'params/fashionmnist/epoch{epoch}_{tag}')
            print("saved params\n")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epochs', default=2000, type=int, help="number of sweeps over the dataset (epochs)")
    ...
    if root_process:
        train_set = datasets.FashionMNIST(root="data/fashionmnist", train=True, transform=transform_ops, download=True)
        test_set = datasets.FashionMNIST(root="data/fashionmnist", train=False, transform=transform_ops, download=True)
    ...
    if not root_process:
        train_set = datasets.FashionMNIST(root="data/fashionmnist", train=True, transform=transform_ops, download=True)
        test_set = datasets.FashionMNIST(root="data/fashionmnist", train=True, transform=transform_ops, download=True)
